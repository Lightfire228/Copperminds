...
impl Watcher {
    ...
    pub async fn next_event(&mut self) -> Option<ModificationType> {
        ...
        while let Some(event) = self.rx.next().await {
            ...
            let Some(_event) = filter_event(event.unwrap()) else {
                continue;
            };

            return Some(ModificationType::Unknown)
    ...
    // fn handle_event(&self, event: Event) -> Option<ModificationType> {
    ...
    //         Ek::Access(_)                    => unreachable!("EventKindMask::CORE"),
    ...
    //             warn!("[{count}] Unknown file watch: {kind:?}, {:?}", );
...
fn _get_id(name: &Path) -> io::Result<FileId> {
...
fn filter_event(event: Event) -> Option<Event> {
    // MAYBE: unify filter logic into one place for both initial vault indexing, and events
    //       - is '.md'
    //       - is hidden
    //       - is '.git'
    //       - is '.obsidian'

    // MAYBE: should probably exclude vault path prefix from filter checking logic
